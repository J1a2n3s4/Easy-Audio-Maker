...
void MoveSlider::setValues(float val, float max)
{
    value = val;
    Max = max;
}

float MoveSlider::getValue()
{
    return value;
}

MoveSlider::MoveSlider(std::string Name, float val, float xPos, float yPos, float max)
{
    Max = max;
    X = xPos;
    Y = yPos;
...
void MoveSlider::Process(sf::RenderWindow* WSK, sf::Vector2i MousePos, sf::Vector2f WinSize)
{
    Track.setPosition(X * (1000 / WinSize.x), Y * (1000 / WinSize.y));
    Track.setOrigin(0, -2.5 * (1000 / WinSize.y));
    name.setPosition((X - 100) * (1000 / WinSize.x), Y * (1000 / WinSize.y));
    ...
    Track.setSize(sf::Vector2f(WinSize.x - 602, 10));
}
